...
void test_queue_null_length(){
	...
	int error_value_res = queue_length(queue);
	TEST_ASSERT(error_value_res == -1);
}
...
void test_delete_and_not_found(void)
{
	int set[3] = {0, 1, 2};
	int doesnt_exist_value = 100;
	queue_t queue = queue_create();

	fprintf(stderr, "*** TEST delete and not found value ***\n");

	for (int i = 0; i < 3; i++) {
		queue_enqueue(queue, &set[i]);	
	}

	int error_value_res = queue_delete(queue, &doesnt_exist_value);
	TEST_ASSERT(error_value_res == -1);
}
